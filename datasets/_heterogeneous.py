...
class HeterogeneousSCM(torch.utils.data.Dataset):
    ...
    def get_interGT(self, u_g):
        ...
        for obs_i, pa_i in zip(nodes_list, parents_list):
            if len(pa_i) == 0:
                xi, ui = self.sample_obs(obs_id=obs_i, n_samples=n_samples, u=u_g[:,obs_i].reshape(-1,1))
            else:
                parents_dict = {self.nodes_list[pa_ij]: x[self.nodes_list[pa_ij]] for pa_ij in pa_i}
                xi, ui = self.sample_obs(obs_id=obs_i, parents_dict=parents_dict, n_samples=n_samples, u=u_g[:,obs_i].reshape(-1,1))

            x[self.nodes_list[obs_i]] = xi.reshape(n_samples,-1)
            u[self.nodes_list[obs_i]] = ui.reshape(n_samples,1)
        x = np.concatenate([x[k] for k in self.nodes_list], axis=1)
        u = np.concatenate([u[k] for k in self.nodes_list], axis=1)
        return x, u
