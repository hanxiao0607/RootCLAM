...
def split_dataset(data_module, name='loan', training_size=5000, seed=0):
    ...
    if name == 'loan':
        ...
        test = pd.concat([df_n.iloc[(size_train_n + size_valid_n):(size_train_n + size_valid_n + size_test_n)] \
                             , df_ab.iloc[:size_test_ab]], ignore_index=True)
        ...
        return thres_n, thres_ab, train, valid, test
...
def prepare_adcar_training_data(df, y_pred, data_module, data='loan'):
    if data == 'loan':
        df['pred'] = y_pred
        df = df.loc[df['pred'] == 1]
        ...
        return x_train, u_train, x_valid, u_valid, x_test, u_test, df
    elif data == 'adult':
        df['pred'] = y_pred
        df = df.loc[df['pred'] == 1]
        df = df.sample(len(df), random_state=42).reset_index(drop=True)
        ...
        u_train = df_train.iloc[:, 25:-2].values
        ...
        u_valid = df_valid.iloc[:, 25:-2].values
        ...
        u_test = df_test.iloc[:, 25:-2].values
        return x_train, u_train, x_valid, u_valid, x_test, u_test, df
